...
def plot_residuals(truths, preds, model_name=None):
    ...
    # X Residuals
    # 1. Plot the histogram and save the bar heights/edges
    ...
    # 2. Get the center of each histogram bar for the curve fit
    bin_centers_1 = (bins_1[:-1] + bins_1[1:]) / 2
    
    # 3. Fit the Gaussian to the histogram bars
    # p0 is our initial guess: [Height, Mean, Std]
    initial_guess_1 = [max(counts_1), 0, np.std(residual_1)]
    popt_1, _ = curve_fit(gaussian, bin_centers_1, counts_1, p0=initial_guess_1)
    amp_1, mu_1, std_1 = popt_1 # Extract the fitted parameters
    
    # 4. Plot the fitted curve
    xmin_1, xmax_1 = axes[1,0].get_xlim()
    x_1 = np.linspace(xmin_1, xmax_1, 200)
    axes[1,0].plot(x_1, gaussian(x_1, *popt_1), 'k', linewidth=2, label='Core Gaussian\n' + rf'$\mu={mu_1:.2f}$, $\sigma={std_1:.2f}$')
    ...
    axes[1,1].plot(x_2, gaussian(x_2, *popt_2), 'k', linewidth=2, label='Core Gaussian\n' + rf'$\mu={mu_2:.2f}$, $\sigma={std_2:.2f}$')    
...
def plot_spatial_resolution(truths, preds, model_name=None):
    ...
    print(f"Median Spatial Resolution: {median_resolution:.2f} units")
...
def plot_spatial_bias(truths, preds, model_name=None):
    ...
    # Calculate Radius from the center of the detector (R = sqrt(X^2 + Y^2))
    R_true = np.sqrt(X_true**2 + Y_true**2)


    # ==========================================
    # Plot: 2D Spatial Error Heatmap (Hexbin)
    # ==========================================
    # Hexbin groups coordinates into hexagons and calculates the median error in that hex
    ...
    plt.axis('equal') # Keep detector circular
...
#     # ==========================================
#     # Plot 1: Error vs. Radial Distance
#     # ==========================================
#     max_radius = int(np.max(R_true))
#     # Create bins for the radius (e.g., chunks of 50 meters/units)
#     bins = np.arange(0, max_radius + 50, 50) 
    
#     # Calculate median error for each radial slice
#     median_errors, bin_edges, _ = binned_statistic(
#         x=R_true, values=distance_errors, statistic='median', bins=bins
#     )
#     bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
    
#     axes[0].plot(bin_centers, median_errors, marker='o', color='crimson', linewidth=2, markersize=8)
#     axes[0].scatter(R_true, distance_errors, alpha=0.1, color='gray', s=10, zorder=0)
    
#     axes[0].set_title('Spatial Bias: Error vs. Radius')
#     axes[0].set_xlabel('True Radius from Detector Center ($R$)')
#     axes[0].set_ylabel('Median Distance Error ($\Delta d$)')
#     axes[0].set_xlim(0, bins[-1])
#     axes[0].set_ylim(0, np.percentile(distance_errors, 95)) # Zoom in to ignore massive outliers
#     axes[0].grid(True, linestyle='--', alpha=0.7)
    
#     # ==========================================
#     # Plot 2: 2D Spatial Error Heatmap (Hexbin)
#     # ==========================================
#     # Hexbin groups coordinates into hexagons and calculates the median error in that hex
#     hb = axes[1].hexbin(X_true, Y_true, C=distance_errors, reduce_C_function=np.median, 
#                         gridsize=20, cmap='inferno', mincnt=1, alpha=0.9)
    
#     cbar = plt.colorbar(hb, ax=axes[1])
#     cbar.set_label('Median Distance Error ($\Delta d$)')
    
#     axes[1].set_title('Top-Down Spatial Error Heatmap')
#     axes[1].set_xlabel('True X Position')
#     axes[1].set_ylabel('True Y Position')
#     axes[1].axis('equal') # Keep detector circular
...
# ==========================================
# 2. THE MASTER EVALUATION FUNCTION
# ==========================================

def evaluate_model(model, test_loader, device, model_name=None):
    """
    Extracts predictions, truths, and graph features from the model,
    then passes them to dedicated plotting functions.
    """
    model.eval() 
    all_preds, all_truths, all_nhits = [], [], []
    
    print("Extracting predictions from the dataset...")
    with torch.no_grad(): 
        for batch_spectra, batch_labels in test_loader:
            batch_spectra = batch_spectra.to(device)
            
            # 1. Forward Pass
            preds = model(batch_spectra)
            
            # 2. Extract N_hits using bincount
            nhits = torch.bincount(batch_spectra.batch, minlength=batch_spectra.num_graphs)
            
            # 3. Move to CPU and store
            all_preds.append(preds.cpu().numpy())
            all_truths.append(batch_labels.cpu().numpy())
            all_nhits.append(nhits.cpu().numpy())
            
    # Stack everything into flat, 2D numpy arrays
    all_preds = np.vstack(all_preds)
    all_truths = np.vstack(all_truths)
    all_nhits = np.concatenate(all_nhits)
    
    print("Generating and saving plots...")
    
    # Pass the data to your modular plotting functions
    plot_residuals(all_truths, all_preds, model_name)
    plot_spatial_resolution(all_truths, all_preds, model_name)
    plot_nhits_performance(all_truths, all_preds, all_nhits, model_name)
    plot_error_vector_map(all_truths, all_preds, model_name)
    plot_spatial_bias(all_truths, all_preds, model_name)
...
class AngularMSELoss(nn.Module):
    ...
    def forward(self, preds, targets):
        """
        Calculates the Mean Squared Error using the shortest angular path.
        Assumes preds and targets are in RADIANS.
        """
        # 1. Calculate the raw linear difference
        diff = preds - targets
        
        # 2. Wrap the difference to always be between -pi and +pi
        # This forces the math to take the shortest path around the circle!
        wrapped_diff = (diff + torch.pi) % (2 * torch.pi) - torch.pi
        
        # 3. Calculate standard MSE on the wrapped differences
        loss = torch.mean(wrapped_diff ** 2)
